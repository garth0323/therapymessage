...
(function() {
  ...
  Growlyflash.Alert = (function() {
    function Alert(flash, options) {
      this.flash = flash;
      this.align = options.align, this.delay = options.delay, this.dismiss = options.dismiss, this.msg = options.msg, this.spacing = options.spacing, this.target = options.target, this.type = options.type, this["class"] = options["class"];
      this.el = ($('<div>', {
        "class": this._classes().join(' '),
        html: "" + (this._dismiss()) + this.msg
      })).appendTo(this.target);
      this.el.css(this._calc_position()).toggleClass('in');
      if (this.delay > 0) {
        this.el.delay(this.delay).fadeOut(function() {
          return ($(this)).remove();
        });
      }
    }
    ...
    Alert.prototype._calc_offset = function() {
      var amount;
    ...
    Alert.prototype._calc_position = function() {
      var styles;
      styles = {};
      styles.top = "" + (this._calc_offset()) + "px";
      if (this.align === 'center') {
        styles.marginLeft = "-" + (this.el.outerWidth() / 2) + "px";
      }
      return styles;
    };
  ...
  Growlyflash.Listener = (function() {
    ...
    Stack = (function(_super) {
      ...
      Stack.prototype.has_uniq_in = function(alerts, counter) {
        ...
        for (id in alerts) {
          item = alerts[id];
          if (typeof (_base = recent[id]).isnt_equal === "function" ? _base.isnt_equal(item) : void 0) {
            counter++;
          }
        }
        return counter > 0;
    ...
    process = function(alerts) {
      ...
      for (type in alerts) {
        msg = alerts[type];
        if (msg != null) {
          _results.push(new Growlyflash.FlashStruct(msg, type));
        }
      }
      return _results;
    };
